...
```text
安装 product-teardown：https://raw.githubusercontent.com/Fable-Forge/product-teardown/main/docs/install.md
```
...
从 `https://github.com/Fable-Forge/product-teardown` 安装 `product-teardown`，验证目标目录和 `SKILL.md`，并诚实报告哪些 Agent 已可发现。
...
```bash
npx skills add Fable-Forge/product-teardown
```
...
| 运行时 | 目标目录 |
|---|---|
| Codex | `~/.codex/skills/product-teardown/` |
| Claude Code | `~/.claude/skills/product-teardown/` |
| Agent Skills | `~/.agents/skills/product-teardown/` |
